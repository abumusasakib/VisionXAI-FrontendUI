...
final _logger = Logger('PaletteSettingsCubit');
...
class PaletteSettingsCubit extends Cubit<PaletteSettingsState> {
  ...
  Future<void> loadOverrides() async {
    ...
    try {
      ...
      _logger.info('loadOverrides: loaded overrides=$ov');
      ...
      // preview timestamps.
      _logger.info('loadOverrides: storedTimestamp=$storedTs');
      emit(
        state.copyWith(
      ...
      _logger.warning('loadOverrides: error $e');
      emit(
        state.copyWith(
          status: PaletteSettingsStatus.failure,
          message: e.toString(),
        ),
      );
    }
  ...
  Future<void> saveOverrides(Map<String, String> overrides) async {
    emit(state.copyWith(status: PaletteSettingsStatus.loading));
    try {
      _logger.info('saveOverrides: saving overrides=$overrides');
      ...
      _logger.info('saveOverrides: saved and refreshed PaletteCubit');
      emit(
        state.copyWith(
          status: PaletteSettingsStatus.success,
          overrides: overrides,
      ...
      _logger.warning('saveOverrides: error $e');
      emit(
        state.copyWith(
          status: PaletteSettingsStatus.failure,
          message: e.toString(),
  ...
  Future<void> clearOverrides() async {
    emit(state.copyWith(status: PaletteSettingsStatus.loading));
    try {
      _logger.info('clearOverrides: clearing persisted overrides');
      ...
      _logger.info('clearOverrides: cleared and refreshed PaletteCubit');
      emit(
        state.copyWith(
          status: PaletteSettingsStatus.success,
          overrides: null,
      ...
      _logger.warning('clearOverrides: error $e');
      emit(
        state.copyWith(
          status: PaletteSettingsStatus.failure,
          message: e.toString(),
  ...
  void updatePreviewColor(String key, String? hex) {
    ...
    if (hex == null || hex.trim().isEmpty) {
      current.remove(key);
      _logger.info('updatePreviewColor: removed preview for key=$key');
    } else {
      current[key] = hex.trim();
      _logger.info('updatePreviewColor: set preview $key=$hex');
    }
    emit(
      state.copyWith(
        previewColors: current,
        previewColorsUpdatedAt: DateTime.now(),
      ),
    );
  }
}
