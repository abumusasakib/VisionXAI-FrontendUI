...
final _logger = Logger('ImageCaptionRepoImpl');
...
class ImageCaptionRepoImpl implements ImageCaptionRepo {
  ...
  Future<Either<Exception, ImageCaptionEntityGroup>> call(
    ...
    try {
      ...
      final response = await _remote.captionImage(
        multipart,
        cancelToken: cancelToken,
      );
      ...
      try {
        _logger.severe('DioException in ImageCaptionRepoImpl.call: $e', e, st);
        if (e.response != null) {
          _logger.severe('DioException response: ${e.response}');
        }
      } catch (_) {}
      // Propagate DioException as-is (it implements Exception)
      return Left(e);
    } catch (e, st) {
      _logger.severe('Exception in ImageCaptionRepoImpl.call: $e', e, st);
