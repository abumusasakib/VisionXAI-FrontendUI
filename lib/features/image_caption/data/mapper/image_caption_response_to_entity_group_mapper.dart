...
final Logger _logger = Logger('ImageCaptionResponseToEntityGroupMapper');
...
class ImageCaptionResponseToEntityGroupMapper
  ...
  ImageCaptionEntityGroup performMap(ImageCaptionResponseDto from) {
    ...
    if (status == 200 && response.caption.isNotEmpty) {
      ...
      try {
        attentionImageBytes = decodeAttentionImage(
          response.attentionImageBytes,
        );
      } catch (_) {
      ...
      final enrichedAttributes = baseEntity.attributes.copyWith(
        ...
        attentionColors: response.attentionColors,
        attentionColorMap: response.attentionColorMap,
        tokenScores: response.tokenScores,
        attentionMeans: response.attentionMeans,
        attentionImage: response.attentionImage,
        attentionShape: response.attentionShape,
      ...
      try {
        ...
        buffer.writeln(
          ' tokens: ${tokensLen ?? 0}, token_scores: ${tokenScoresLen ?? 0}, colors: ${enrichedAttributes.attentionColors?.length ?? 0}',
        );
